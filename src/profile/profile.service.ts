...
export class ProfileService {
    ...
    async unfollowProfile(userId: number, profileUsername: string): Promise<ProfileType> {
        const user = await this.userRepository.findOne({
            where: {
                username: profileUsername
            }
        ...
        if (userId === user.id) {
            throw new HttpException('Follower and Unfollowing cant be equal', HttpStatus.BAD_REQUEST);
        }
